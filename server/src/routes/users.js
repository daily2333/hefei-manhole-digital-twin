...
router.post('/', authMiddleware, adminOnly, (req, res) => {
  ...
  ).run(id, username, hashedPassword, display_name || '', role || 'operator', status || 'active', email || '', phone || '');
...
// Update user (requires auth, can only update self unless admin)
router.put('/:id', authMiddleware, (req, res) => {
  try {
    ...
    // Non-admin users can only update their own profile (not role/status)
    ...
    const allowedFields = isAdmin
      ? ['username', 'password', 'display_name', 'role', 'status', 'email', 'phone']
