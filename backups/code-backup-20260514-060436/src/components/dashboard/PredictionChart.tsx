...
const PredictionChart: React.FC<PredictionChartProps> = ({ manholeId }) => {
  ...
  // 生成阈值配置对象
  const getThresholdsConfig = () => {
    const { normalRange, warningRange, criticalRange } = dataTypeMap[dataType];
    
    // 确保有数据时才返回annotations
    if (!predictionData.length || !showThresholds) {
      return [];
    }
    ...
    return [
      { 
        type: 'line',
        start: ['min', warningRange[0]],
        end: ['max', warningRange[0]],
        style: {
          stroke: '#faad14',
          lineDash: [4, 4],
        }
      ...
      { 
        type: 'line',
        start: ['min', criticalRange[0]],
        end: ['max', criticalRange[0]],
        style: {
          stroke: '#f5222d',
          lineDash: [4, 4],
        }
  ...
  const getChartConfig = () => {
    const typeInfo = dataTypeMap[dataType];
    ...
    const config = {
      ...
      meta: {
        ...
        value: {
          alias: typeInfo.label,
          formatter: (v: number) => `${v.toFixed(1)}${typeInfo.unit}`,
        },
        type: {
          alias: '数据类型',
        },
      ...
      yAxis: {
        title: {
          text: `${typeInfo.label} (${typeInfo.unit})`,
        },
        min: predictionData.length ? Math.min(...predictionData.map(d => d.value)) * 0.9 : undefined,
        max: predictionData.length ? Math.max(...predictionData.map(d => d.value)) * 1.1 : undefined,
        nice: true
      },
      tooltip: {
        showCrosshairs: true,
        ...
        formatter: (datum: any) => {
          return { 
            name: datum.type,
            value: `${datum.value} ${typeInfo.unit}`,
          };
        },
        customContent: (title: string, items: any[]) => {
          ...
          try {
            ...
            if (!isNaN(Number(title))) {
              // 如果是数字字符串，尝试作为时间戳解析
              titleDate = new Date(parseInt(title));
            } else {
              // 否则尝试直接作为日期字符串解析
              titleDate = new Date(title);
            }
            
            // 检查解析结果是否有效
            if (!isNaN(titleDate.getTime())) {
              formattedTitle = titleDate.toLocaleString('zh-CN', { 
                year: 'numeric', 
                month: 'numeric', 
                day: 'numeric', 
                hour: 'numeric', 
          ...
          return (
            <div style={{ padding: '8px 12px' }}>
              <div style={{ marginBottom: '8px', fontWeight: 'bold' }}>{formattedTitle}</div>
              {items?.map((item, index) => {
                const color = item.name === '预测数据' ? '#91caff' : typeInfo.color;
                return (
                  <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: color, marginRight: '8px' }}></div>
                    <div style={{ marginRight: '12px' }}>{item.name}:</div>
      ...
      annotations: getThresholdsConfig(),
      smooth: true,
      animation: {
        appear: {
          animation: 'wave-in',
      ...
      point: {
        size: 4,
        shape: (datum: any) => {
          return datum.type === '实际数据' ? 'circle' : 'diamond';
        },
        style: (datum: any) => {
          if (datum.type === '预测数据') {
            return {
