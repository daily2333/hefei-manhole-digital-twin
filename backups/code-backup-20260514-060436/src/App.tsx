...
const App: React.FC = () => {
  ...
  const fetchManholeData = useCallback(async () => {
    try {
      ...
      const enhancedManholes = generateEnhancedManholes(30);
      const nextRealTimeDataMap = new Map<string, ManholeRealTimeData>();

      enhancedManholes.forEach((manhole) => {
        if (manhole.latestData) {
          nextRealTimeDataMap.set(manhole.id, manhole.latestData);
        }
      });

      const nextAlarms = generateMockAlarms(enhancedManholes);
      setManholes(enhancedManholes);
      setRealTimeDataMap(nextRealTimeDataMap);
      setAlarms(nextAlarms);
      setNotifications(getRandomInt(1, 10));
    } catch (error) {
